...
| 027 | Rename `project.qiita_id` → `external_project_id` | Renamed DB column in `schema.sql` and updated 4 reconstruction views (`control_project_associations`, `omnibus_sample_context`, `omnibus_pacbio_absquant_v11_bioinformatics`, `omnibus_standard_metag_v90_bioinformatics`) to source the new column name; CSV emit aliases (`QiitaID`, `primary_qiita_study`, `secondary_qiita_studies`) preserved verbatim; new patch `001_rename_qiita_id_to_external_project_id.sql` carries the change to existing v0 DBs via `ALTER TABLE ... RENAME COLUMN` (SQLite auto-rewrites dependent view bodies); `schema_v0.sql` left untouched; INSERT column lists updated in `db.py` and 3 test files; diagram updated |
