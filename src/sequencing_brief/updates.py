...
_PLATFORM_TABLES = {
    UPDATE_PLATFORM_ILLUMINA: (TABLE_ILLUMINA_SAMPLE, "illumina_sample_id"),
    UPDATE_PLATFORM_TELLSEQ: (TABLE_TELLSEQ_SAMPLE, "tellseq_sample_id"),
}
...
def _set_biosample_accession(
    ...
    cur.execute(
        """
        SELECT DISTINCT ins.input_sample_id, ins.biosample_accession
        FROM input_sample ins
        JOIN compression_sample cs ON cs.input_sample_id = ins.input_sample_id
        JOIN prepped_sample prs ON prs.compression_sample_id = cs.compression_sample_id
    ...
    input_sample_id, old_accession = matches[0]

    # Apply the update and write the audit row in a single transaction.
    try:
        cur.execute(
            f"UPDATE {TABLE_INPUT_SAMPLE} "
            f"SET {DB_COL_BIOSAMPLE_ACCESSION} = ? WHERE input_sample_id = ?",
            (accession, input_sample_id),
        )
        _log_change(
            conn,
            TABLE_INPUT_SAMPLE,
...
def update_lane(
    ...
    - a post-update state that would collide with the unique
      ``(prepped_sample_id, COALESCE(lane, -1))`` index (i.e. a
...
def _update_lane(
    ...
    # Verify the unique (prepped_sample_id, lane) index will not
    # collide.  When from_lane == to_lane there is no logical change.
    if from_lane != to_lane:
        cur.execute(
            f"SELECT COUNT(DISTINCT prepped_sample_id) FROM {table} "
            "WHERE COALESCE(lane, -1) = COALESCE(?, -1) "
            "AND prepped_sample_id IN ("
            f"  SELECT prepped_sample_id FROM {table} "
            "  WHERE COALESCE(lane, -1) = COALESCE(?, -1)"
            ")",
            (to_lane, from_lane),
        )
    ...
    try:
        ...
        for row_id, old_lane in affected:
            _log_change(
                conn,
                table,
                row_id,
